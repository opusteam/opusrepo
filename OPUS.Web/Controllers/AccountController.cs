...
namespace OPUS.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    return RedirectToAction("Index","Home", new { area = user.DepartmentName });
                }
                else
                {
        ...
        [AllowAnonymous]
        public ActionResult TestUrlAction(ClientViewModel _Client)
        {
            return View();
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
               var user = new IdentityUser() { UserName = model.UserName, Email = model.Email, ContactNo = model.ContactNo, DepartmentName = model.DepartmentName.ToString(), UserRole = model.UserRole.ToString() };
               var result = await _userManager.CreateAsync(user, model.Password);
               
                if (result.Succeeded)
                {
                    user = _userManager.FindByName(model.UserName);
                    _userManager.AddToRole(user.Id, model.UserRole.ToString());
                   // await SignInAsync(user, isPersistent: false);
